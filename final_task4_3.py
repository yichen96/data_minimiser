...
tau_l = np.arange(0.36, 0.53, 0.01)
signalfrac_l = np.arange(0.4, 1.01, 0.01)  # signal fraction is only defined between 0 and 1
...
plt.contourf(X, Y, Z, 10, alpha=0.5)


startpoint = np.array([0.4, 0.4])
...
plt.plot(path_grad[:, 0], path_grad[:, 1], 'ro-')
plt.plot(path_newton[:, 0], path_newton[:, 1], 'go-')
# plt.plot(path_quasi[:, 0], path_quasi[:, 1], 'c.-')
plt.show()
